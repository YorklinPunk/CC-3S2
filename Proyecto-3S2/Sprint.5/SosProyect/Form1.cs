...
namespace SosProyect
{
    public partial class Form1 : Form
    {
        ...
        //RUTA DE ARCHIVO
        string routeDektop = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
        string nameFile = "SosGame";
        string routeFile = "";
        string docFile = "";
        ...
        private TextBox createTextBox(int i, int j)
        {
            ...
            enterEventHandler = (sender, e) =>
            {
                ...
                //UNA VEZ LLENADO EL CAMPO PROCEDEMOS A GUARDAR EN EL TXT
                if(checkBox1.Checked)
                {
                    //LLENAR EL TXT 
                    writeOnTxt(getTurn(), textBox.Text, i, j);
                }
                //
        ...
        public void verifyEnd()
        {
            ...
            if(radioButton5.Checked)
            {
                ...
                {
                    endMessage = $"{getTurn()} player won..!!";
                    MessageBox.Show(endMessage);
                    writeOnTxt(endMessage,"X",-1,-1); //X PARA PARA CONDICIONAL EL LA FUNCION
                    redComputer = false;
                    blueComputer = false;
                    openFile(); //ABRIR TEXTO
                }
            ...
            else
            {
                if (totalMovs == int.Parse(textBox10.Text) * int.Parse(textBox10.Text))
                {
                    if(contRed > contBlue)
                    {
                        endMessage = $"Red player won..!! Red: {contRed} Blue: {contBlue}";
                    }
                    else if (contBlue > contRed)
                    {
                        endMessage = $"Blue player won..!! Red: {contBlue} Blue: {contRed}";    
                    }
                    else
                    {
                        endMessage = $"Draw..!! Red: {contBlue} Blue: {contRed}";
                    }
        ...
        */
        ...
        {
            string lastSize = textBox10.Text;
            textBox10.Text = "";
            //eliminamos todos movimientos ya realizados y creando la tabla 
            deleteTextBox();
            cleanText();
            textBox10.Text = lastSize;
            //RESETEAR LOS VALORES INCIALES COMO LOS CONTADORES
            lastTurn = "Blue";
            totalMovs = 0;
            contBlue = 0;
            contRed = 0;
        ...
        {
            routeFile = Path.Combine(routeDektop, nameFile);
            docFile = Path.Combine(routeFile, "SosGame.txt");
        }
        public void verifyExistFile()
        {
            ...
            {
                using (StreamWriter sw = File.CreateText(docFile))
                {
                    sw.Close();
                }
            }
        }
        public void writeOnTxt(string stringText, string letter, int i, int j)
        {
            using (StreamWriter sw = File.AppendText(docFile))
            {
                string moveText = letter == "X" ? stringText : $"El jugador {stringText} selecciono el campo -{i + 1}- , -{j + 1}- y escribio la letra -{letter}-";
                sw.WriteLine(moveText);
                sw.Close();
            }
        }
        public void cleanText()
        {
            File.WriteAllText(docFile, string.Empty);
        }
        public void openFile()
        {
            //ABRIMOS EL DOCUMENTO
            Process.Start("notepad.exe", docFile);
        }
        ...
        {
            //BOTON DE REPLAY 
            //LEER DOCUMENTO TXT Y REALIZAR LOS PASOS
            char separator = '-';
            string[] linesText = readFile();
            //resetGame();
            for (int k = 0; k < linesText.Length - 1; k++)
            {
                string line = linesText[k];
                string[] parts = line.Split(separator);
                int i = int.Parse(parts[1]);
                int j = int.Parse(parts[3]);
                string letter = parts[5];

                setLetterReplay(getTablePosition(i - 1, j - 1), letter);
                searchSOSWithLetter(letter, i - 1, j - 1);
                //TIMER 1 SEGUNDO 
            }
        }
        public void setLetterReplay(int pos,string letter)
        {
            int index = pos;
            TextBox textBox = panel2.Controls.OfType<TextBox>().FirstOrDefault(textBoxReplay => textBoxReplay.Tag != null && (int)textBoxReplay.Tag == index);
            textBox.Text = letter;
        }
    }
}
